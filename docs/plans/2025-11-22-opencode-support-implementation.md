...
> **给 Claude 的提示：** 必需子技能：使用 superpowers:executing-plans 按任务逐步实现此计划。

**目标：** 为 OpenCode.ai 添加完整的 superpowers 支持，使用原生 JavaScript 插件，与现有 Codex 实现共享核心功能。

**架构：** 将通用 skill 发现/解析逻辑提取到 `lib/skills-core.js`，重构 Codex 使用它，然后使用 OpenCode 原生插件 API 构建 OpenCode 插件，包含自定义工具和会话 hooks。
...
function resolveSkillPath(skillName, superpowersDir, personalDir) {
    // 如果存在，剥离 superpowers: 前缀
    const forceSuperpowers = skillName.startsWith('superpowers:');
    const actualSkillName = forceSuperpowers ? skillName.replace(/^superpowers:/, '') : skillName;

    // 首先尝试个人 skills（除非明确指定 superpowers:）
...
```javascript
    'session.started': async () => {
      ...
      const toolMapping = `
**OpenCode 工具映射：**
当 skills 引用你没有的工具时，请替换为 OpenCode 等价物：
- \`TodoWrite\` → \`update_plan\`（你的规划/任务跟踪工具）
- 带 subagents 的 \`Task\` 工具 → 使用 OpenCode 的 subagent 系统（@mention 语法或自动调度）
...
**Skills 命名：**
- Superpowers skills：\`superpowers:skill-name\`（来自 ~/.config/opencode/superpowers/skills/）
- 个人 skills：\`skill-name\`（来自 ~/.config/opencode/skills/）
...
**以下是您的 'superpowers:using-superpowers' skill 的完整内容 - 您使用 skills 的入门指南。对于所有其他 skills，请使用 'use_skill' 工具：**
...
重启 OpenCode 以加载插件。在下一个会话中，您应该看到：
...
个人 skills 在名称相同时会覆盖同名 superpowers skills。
...
1. 验证 skills 目录存在：`ls ~/.config/opencode/superpowers/skills`
2. 使用 `find_skills` 工具查看发现了什么
3. 检查文件结构：每个 skill 应该有一个 `SKILL.md` 文件

### 工具映射问题

当 skill 引用您没有的 Claude Code 工具时：
- `TodoWrite` → 使用 `update_plan`
- 带 subagents 的 `Task` → 使用 `@mention` 语法调用 OpenCode subagents
...
找到关于支持平台的部分（在文件中搜索"Codex"），然后在其后添加：
...
- **OpenCode 支持**：OpenCode.ai 的原生 JavaScript 插件
  ...
  - 共享核心模块（`lib/skills-core.js`）用于代码重用
...
这些步骤需要已安装 OpenCode，不属于自动化实现的一部分：
...
6. **测试个人 skills**：创建一个个人 skill 并验证它覆盖核心
...
- [ ] Codex 命令仍然可用（find-skills、use-sill、bootstrap）
